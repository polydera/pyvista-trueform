...
terrain. The torus itself is the thing grabbed: an affine transform
widget anchors its gizmo at the ring's center, an arrow drag slides the
torus and a circle drag tilts it, and every motion hands the actor's
accumulated transform to the callback, which applies it to the torus
points and asks the accessor for the field again over all 122k surface
points; on release the gizmo re-anchors to wherever the ring now sits.
The color limits stay fixed at the tube radius — the deepest any point
can sit inside the tube — so the warm pole saturates exactly at the
tube core and dragging never rescales the ramp.
...
def main():
    ...
    actor = plotter.add_mesh(torus, color=_theme.ROSE,
                             style="wireframe", line_width=1.0, opacity=0.5,
                             render_lines_as_tubes=False, lighting=False)

    def resample(matrix):
        probe = torus.transform(matrix, inplace=False)
        hills.point_data["signed distance"] = \
            hills.trueform.signed_distance(probe)

    def settle(matrix):
        resample(matrix)
        widget.origin = actor.center
        plotter.render()

    widget = plotter.add_affine_transform_widget(
        actor, interact_callback=resample, release_callback=settle,
        axes_colors=(_theme.AMBER, _theme.PURPLE, _theme.BLUE))
    plotter.add_text("grab the torus: signed distance recolors live",
